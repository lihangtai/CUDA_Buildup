...
void initialzeMatrix(std::vector<float>& matrix, int rows, int cols){
    std::random_device rd;
    std::mt19937 mt(rd());
    std::uniform_real_distribution<> dis(-1.5, 1.5);
    for(int i =0;i<rows*cols;++i){
        matrix[i] = static_cast<float>(dis(mt));
...
int main(){
    ...
    initialzeMatrix(h_A, M, K);
    initialzeMatrix(h_B, K, N);
    ...
    CHECK_CUDA(cudaMemcpy(d_A, h_A.data(), sizeof(float) * M * K, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_B, h_B.data(), sizeof(float) * K * N, cudaMemcpyHostToDevice));
    ...
    half alpha_half = __float2half(alpha), beta_half = __float2half(0.0f);
    ...
    std::cout << "cuBlAS fp32 average time" << cublas_fp32_time << std::endl;
    ...
    float cublastLt_fp32_time = benchmark_kernel([&](){
    ...
    std::cout << "cuBLASLt FP32 average time: " << cublastLt_fp32_time << " ms" << std::endl;
